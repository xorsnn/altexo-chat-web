...
uniform float icosahedronRadius;
...
vec3 getNewPos(vec3 currentPos) {
  ...
  return vec3(
    // currentPos.x*(1.0 + aVertexFFT/255.0*(1.0 - distanceK)),
    // currentPos.y*(1.0 + aVertexFFT/255.0*(1.0 - distanceK)),
    // currentPos.z*(1.0 + aVertexFFT/255.0*(1.0 - distanceK)));
...
void main(void) {
  ...
  vUvy = (icosahedronRadius*(1.0 + distanceK) - (modelMat.y - centerY))/(icosahedronRadius*(1.0 + distanceK));
